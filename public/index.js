...
getRequestButton.addEventListener("click", event => {
    ...
    fetch(scoresURL)                           // Note 1 (see below)- make a request
      .then(response => response.json())       // Note 2 - returning anothe promise - convert to json 
      .then(scores => {                        // Note 3 - 
        console.log(scores);
                                               // Note 4
...
function gameOver() {
    let newArbitraryData = {
        name: playerName,
        score: score,
      }
      console.log(newArbitraryData);
    const postRequestOptions = {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
        body: JSON.stringify(newArbitraryData),
    }
    ...
    //request post w/ fetch
    //take the score and json.stringify it
    //push it to the score
    //
